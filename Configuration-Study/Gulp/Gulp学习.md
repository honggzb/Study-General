...
<table border="1">
  ...
  <tr>
    <td>`*`</td> <td>匹配文件路径中的0个或多个字符，但不会匹配路径分隔符，除非路径分隔符出现在末尾</td>
    <td>`**`</td> <td>匹配路径中的0个或多个目录及其子目录,需要单独出现，即它左右不能有其他东西了。如果出现在末尾，也能匹配文件</td>
    <td>`?`</td> <td>匹配文件路径中的一个字符(不会匹配路径分隔符)</td>
    <td>`[...]`</td> <td>匹配方括号中出现的字符中的任意一个，当方括号中第一个字符为^或!时，则表示不匹配方括号中出现的其他字符中的任意一个，类似js正则表达式中的用法</td>
    <td>`!(pattern|pattern|pattern)`</td> <td>匹配任何与括号中给定的任一模式都不匹配的</td>
    <td>`?(pattern|pattern|pattern)`</td> <td>匹配括号中给定的任一模式0次或1次，类似于js正则中的`(pattern|pattern|pattern)?`</td>
    <td>`+(pattern|pattern|pattern)`</td> <td>匹配括号中给定的任一模式至少1次，类似于js正则中的`(pattern|pattern|pattern)+`</td>
    <td>`*(pattern|pattern|pattern)`</td> <td>匹配括号中给定的任一模式0次或多次，类似于js正则中的`(pattern|pattern|pattern)*`</td>
    <td>`@(pattern|pattern|pattern)`</td> <td>匹配括号中给定的任一模式1次，类似于js正则中的`(pattern|pattern|pattern)`</td>
  </tr>
</table>
