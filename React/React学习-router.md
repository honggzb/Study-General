...
**技巧**： 所有组件上层设置一个App组件作为其父组件， 系统的默认路由（即浏览器一访问http://localhost:8080/），默认渲染Home组件
...
class App extends React.Component {
    ...
    render() {
        return (
            <div>{ this.state.initDone ? this.props.children : <div> Loading...</div> }</div>
        )
    }
    componentDidMount(){
        //var self = this;
        setTimeout(() => {   //或用 arrow function
            this.setState({
                initDone: true
            });
        }, 1000);
